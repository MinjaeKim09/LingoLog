...
- Applies a Firestore-backed limit of 60 requests per app/client address per minute.
...
- `APP_STORE_ENVIRONMENT`: `Sandbox` before an App Store app ID exists; `Production` for launch. Production verification retries sandbox only when Apple’s verifier reports an environment mismatch, supporting TestFlight and App Review. Invalid signatures never trigger fallback.
- `DEV_SKIP_APPLE_VERIFICATION`: defaults to `false`; set to `true` only while running local emulators (the server rejects this setting outside the emulator) before App Store subscription verification is configured.
...
Before deployment, enable the Cloud Translation API in the Google Cloud project and restrict the Google API key to that API. Configure Firebase App Check for the iOS app with App Attest (team `5699GDC3LT`, matching the current Firebase registration), then register a debug token in Firebase for each simulator used during development. The iOS app sends the token itself because `translation` is a direct HTTPS function rather than a callable function.
